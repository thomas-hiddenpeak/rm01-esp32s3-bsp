...
- **⚡ 设备电源管理**: AGX、LPMU、Orin、N305设备的电源控制和状态监控
- **🔋 电源监控系统**: 供电电压监测、电源芯片数据读取、功率监控、UART协议解析
...
#### 电源监控命令
- `power <action>` - 电源监控和电源芯片数据读取 (详细帮助: `power help`)
  - `power status` - 显示完整电源系统状态
  - `power voltage` - 读取供电电压 (GPIO18 ADC, 分压比11.4:1)
  - `power read [timeout_ms]` - 读取电源芯片数据 (GPIO47 UART, 9600波特率)
...
**电源监控功能特性:**
- **GPIO18**: 供电电压监测，通过ADC2_CHANNEL_7读取，支持高电压分压检测
- **GPIO47**: 电源诱骗芯片UART接收，9600波特率，支持实时电压电流功率数据
- **数据格式**: `[0xFF帧头][电压][电流][CRC]` 4字节协议
- **后台监控**: 支持电压变化阈值触发的自动数据读取
- **调试工具**: 完整的UART诊断和协议分析功能
