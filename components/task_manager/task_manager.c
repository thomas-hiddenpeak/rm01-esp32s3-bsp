...
// 适配器函数声明(用于解决函数签名不匹配问题)
static esp_err_t device_interface_init_adapter(void);
static esp_err_t ethernet_interface_init_adapter(void);
static esp_err_t console_interface_init_adapter(void);
static esp_err_t console_interface_deinit_adapter(void);
static esp_err_t system_monitor_init_adapter(void);
...
esp_err_t task_manager_start_task(const char *task_name)
{
    ...
    if (task->config.type == TASK_TYPE_BACKGROUND && task->config.stack_size > 0) {
        // 注意：这里需要一个适配器函数来处理函数签名不匹配
        // 暂时跳过FreeRTOS任务创建，只调用启动函数
        ESP_LOGW(TAG, "Background task creation not implemented yet for '%s'", task_name);
    }
...
static esp_err_t register_all_system_tasks(void)
{
    ...
    // 配置管理器任务
    tmgr_task_config_t config_task = {
        .name = "config_manager",
    ...
    // 设备接口任务
    tmgr_task_config_t device_task = {
        ...
        .init_func = device_interface_init_adapter,
        .start_func = NULL,
        .stop_func = NULL,
    ...
    // 以太网接口任务
    tmgr_task_config_t ethernet_task = {
        ...
        .freertos_priority = 5,
        .dependencies = "config_manager",
        .init_func = ethernet_interface_init_adapter,
    ...
    // 硬件控制任务
    tmgr_task_config_t hardware_task = {
    ...
    // 控制台接口任务
    tmgr_task_config_t console_task = {
        ...
        .freertos_priority = 0,
        .dependencies = "",
        .init_func = console_interface_init_adapter,
        .start_func = NULL,
        .stop_func = NULL,
        .deinit_func = console_interface_deinit_adapter
    };
    ...
    // 系统监控任务
    tmgr_task_config_t monitor_task = {
        .name = "system_monitor",
        ...
        .freertos_priority = 3,
        .dependencies = "",
        .init_func = system_monitor_init_adapter,
...
// 实现头文件中声明的公共辅助函数
const char* task_status_to_string(tmgr_task_status_t status)
{
    switch (status) {
...
const char* task_type_to_string(tmgr_task_type_t type)
{
    switch (type) {
...
const char* task_priority_to_string(tmgr_task_priority_level_t priority)
{
    switch (priority) {
...
esp_err_t task_manager_get_config(tmgr_task_manager_config_t *config)
{
    if (!s_state.initialized || config == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
    ...
    memcpy(config, &s_state.config, sizeof(task_manager_config_t));

    xSemaphoreGive(s_state.mutex);
    return ESP_OK;
}

esp_err_t task_manager_get_task_info(const char *task_name, tmgr_task_info_t *info)
{
    if (!s_state.initialized || task_name == NULL || info == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t task_manager_start_all_tasks(void)
{
    ...
    // 启动所有配置为自动启动的任务
    for (int i = 0; i < s_state.config.task_count; i++) {
        if (s_state.config.tasks[i].auto_start) {
            err = task_manager_start_task(s_state.config.tasks[i].name);
            if (err != ESP_OK) {
                ESP_LOGE(TAG, "启动任务 %s 失败: %s", 
                         s_state.config.tasks[i].name, esp_err_to_name(err));
                ret = err; // 记录最后一个错误，但继续启动其他任务
            }
        }
    }
    ...
    xSemaphoreGive(s_state.mutex);
    return ret;
}    memcpy(info, task, sizeof(task_info_t));
...
static esp_err_t device_interface_init_adapter(void)
{
    // 使用默认配置或从config_manager获取配置
    return device_interface_init(NULL); // 假设接受NULL参数
}

static esp_err_t ethernet_interface_init_adapter(void)
{
    // 使用默认配置或从config_manager获取配置  
    return ethernet_interface_init(NULL); // 假设接受NULL参数
}

static esp_err_t console_interface_init_adapter(void)
{
    // 使用默认配置或从config_manager获取配置
    return console_interface_init(NULL); // 假设接受NULL参数
}

static esp_err_t console_interface_deinit_adapter(void)
{
    // 控制台接口可能没有deinit函数，返回成功
    return ESP_OK;
}
...
{
    // 使用默认配置或从config_manager获取配置
    return system_monitor_init(NULL); // 假设接受NULL参数
}
