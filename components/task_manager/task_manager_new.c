...
esp_err_t task_manager_register_task(const task_config_t *task_config)
{
    ...
    task->status = TASK_STATUS_UNINITIALIZED;
...
esp_err_t task_manager_start_task(const char *task_name)
{
    ...
    task->start_time = xTaskGetTickCount() * portTICK_PERIOD_MS;
    ...
    if (task->config.init_func != NULL) {
        ...
        if (ret != ESP_OK) {
            ...
            task->last_error = ret;
            snprintf(task->error_message, sizeof(task->error_message), "Init failed: %s", esp_err_to_name(ret));
            xSemaphoreGive(s_state.mutex);
    ...
    if (task->config.start_func != NULL) {
        ...
        if (ret != ESP_OK) {
            task->status = TASK_STATUS_ERROR;
            task->last_error = ret;
            snprintf(task->error_message, sizeof(task->error_message), "Start failed: %s", esp_err_to_name(ret));
            xSemaphoreGive(s_state.mutex);
...
esp_err_t task_manager_list_tasks(task_info_t *tasks, uint32_t max_tasks, uint32_t *actual_count)
{
    ...
    *actual_count = s_state.config.task_count < max_tasks ? s_state.config.task_count : max_tasks;
    memcpy(tasks, s_state.tasks, (*actual_count) * sizeof(task_info_t));
...
static bool check_dependencies_met(const task_info_t *task)
{
    ...
    // 简单解析依赖（逗号分隔）
    char deps[TASK_DEPENDENCIES_MAX_LEN];
    strcpy(deps, task->config.dependencies);
